...
def similarity(y_pred, y_true, multichannel=False):
    """
    Computes the Structural Similarity Index (SSIM) for 3D volumes or images, which is used to assess the 
    perceptual quality of images by comparing structural similarities.

    Args:
        y_pred (np.ndarray): The predicted volumes or images, of shape (B, D, H, W) or (B, D, H, W, C).
        y_true (np.ndarray): The ground truth volumes or images, of shape (B, D, H, W) or (B, D, H, W, C).
        multichannel (bool, optional): Whether the images are multichannel (color). Default is False for grayscale.

    Returns:
        float: The average SSIM score across all slices.
    """
    # Ensure inputs are numpy arrays for compatibility with the skimage SSIM function
    y_pred = y_pred.detach().cpu().numpy()
    y_true = y_true.detach().cpu().numpy()
    
    # Remove the batch and channel dimensions if present
    if y_pred.ndim == 5:
        y_pred = y_pred[0, 0, :, :, :]  # Consider only the first batch and channel if 5D
        y_true = y_true[0, 0, :, :, :]  # Consider only the first batch and channel if 5D

    ssim_scores = []
    if multichannel:
        # Compute SSIM slice by slice for multichannel images
        for i in range(y_pred.shape[0]):
            score, _ = ssim(y_pred[i], y_true[i], full=True, multichannel=True)  # SSIM for multichannel images
            ssim_scores.append(score)
    else:
        # Compute SSIM slice by slice for grayscale images
        for i in range(y_pred.shape[0]):
            score, _ = ssim(y_pred[i], y_true[i], full=True)  # SSIM for grayscale images
            ssim_scores.append(score)
    
    # Compute the average SSIM score across all slices
