...
def main(args):
    ...
    save_dir = 'midir_1_ncc_{}_diffusion_{}/'.format(weights[0], weights[1])
    ...
    Initialize model
    '''
    model = models.CubicBSplineNet(ndim=3)
    model.cuda()
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        dice_score_all = utils.AverageMeter()
        idx = 0
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                output = model((x,y))
                def_out = transformation.warp(x.cuda().float(), output[2].cuda(), interp_mode='nearest')
    ...
    }, save_dir='experiments/' + save_dir, filename='final_model_midir.pth.tar')
...
if __name__ == '__main__':
    ...
    # Argument parser
    parser = argparse.ArgumentParser(description='Train MIDIR Model for Image Registration')
...
# python train_midir.py --t1_dir ./Registration_Data/T1_Moving/ --dwi_dir ./Registration_Data/Diffusion_Fixed/ --epochs 100 --img_size 128,128,128 --lr 2e-4 --batch_size 2 --cont_training
