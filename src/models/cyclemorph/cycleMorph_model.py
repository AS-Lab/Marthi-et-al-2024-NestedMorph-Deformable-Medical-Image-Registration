...
class cycleMorph(BaseModel):
    ...
    def get_current_visuals(self):
        ...
        real_A = tensor2im(self.input_A[0, 0, int(realSize[2]/2)])
        flow_A = tensor2im(self.flow_A[0, :, int(realSize[2] / 2)])
        fake_B = tensor2im(self.fake_B[0, 0, int(realSize[2]/2)])
        back_A = tensor2im(self.back_A[0, 0, int(realSize[2] / 2)])

        real_B = tensor2im(self.input_B[0, 0, int(realSize[2]/2)])
        flow_B = tensor2im(self.flow_B[0, :, int(realSize[2]/2)])
        fake_A = tensor2im(self.fake_A[0, 0, int(realSize[2] / 2)])
        back_B = tensor2im(self.back_B[0, 0, int(realSize[2] / 2)])
