"""
Inference Script for OASIS Dataset.
...
def main():
    parser = argparse.ArgumentParser(description='Inference on OASIS test set')
    ...
    out_dir = args.output_dir or f'results/OASIS/{args.model_label}'
    ...
    with torch.no_grad():
        for i, (x, y, x_seg, y_seg, pair_id) in enumerate(test_loader):
            pair_id = pair_id[0]
            x, y = x.to(device), y.to(device)
            output = model((x, y))
            warped, flow = output[0], output[1]

            save_nifti(warped, os.path.join(out_dir, 'warped', f'{pair_id}_warped.nii.gz'))

            flow_np = flow.cpu().numpy()[0]
            flow_nii = nib.Nifti1Image(flow_np.transpose(1, 2, 3, 0).astype(np.float32), np.eye(4))
            nib.save(flow_nii, os.path.join(out_dir, 'flow', f'{pair_id}_flow.nii.gz'))

            if x_seg.sum() > 0:
                warped_seg = stn_nearest(x_seg.to(device).float(), flow)
                save_nifti(warped_seg, os.path.join(out_dir, 'warped_seg', f'{pair_id}_warped_seg.nii.gz'))

            if (i + 1) % 10 == 0 or (i + 1) == len(test_loader):
                print(f"  [{i+1}/{len(test_loader)}] {pair_id}")
