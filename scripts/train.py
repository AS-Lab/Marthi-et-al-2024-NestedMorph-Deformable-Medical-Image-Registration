...
def train_model(t1_dir, dwi_dir, model_label, epochs, img_size, lr, batch_size, cont_training):
    ...
    if model_label == "MIDIR":
        ...
        model = CubicBSplineNet(ndim=3)  # Initialize MIDIR model
        ...
        model = TransMorph(config)  # Initialize TransMorph model
    ...
    print(f"Number of parameters in the model: {num_params / 1e6:.2f} million parameters")  # Print the number of parameters
    ...
    for epoch in range(epoch_start, epochs):
        ...
        idx = 0  # Batch index
        print(f"Number of batches in train_loader: {len(train_loader)}")  # Print the number of batches
