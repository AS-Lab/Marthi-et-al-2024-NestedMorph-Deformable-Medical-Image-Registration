...
def train_model(t1_dir, dwi_dir, model_label, epochs, img_size, lr, batch_size, cont_training, device):
    ...
    # Load all .pkl or .nii.gz files in the T1 and DWI directories
    t1_files = sorted(glob.glob(t1_dir + '*.pkl') + glob.glob(t1_dir + '*.nii.gz'))   # FIX: support both formats
    dwi_files = sorted(glob.glob(dwi_dir + '*.pkl') + glob.glob(dwi_dir + '*.nii.gz'))
    ...
    last_def_out = None  # FIX: initialise before epoch loop to prevent UnboundLocalError
    last_y = None
    last_x = None
