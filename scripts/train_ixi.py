"""
Training Script for IXI Dataset (Atlas-to-Patient Brain MRI Registration).

Uses the same model initialization, losses, and training loop as the original train.py,
but with the IXI atlas-to-patient dataset instead of paired T1/DWI.
...
from src.data.ixi_dataset import IXIBrainDataset
...
def train_model_ixi(data_dir, atlas_path, model_label, epochs, img_size, lr,
                     batch_size, cont_training, device):
    """
    Train a registration model on the IXI dataset (atlas-to-patient).
    Uses the same training loop structure as the original train.py.
    """
    weights = [1, 1]
    save_dir = f'IXI_{model_label}_ncc_{weights[0]}_diffusion_{weights[1]}/'
    ...
    '''
    Initialize model based on model_label (same as original train.py)
    '''
    ...
    train_set = IXIBrainDataset(
        data_dir=train_dir,
        atlas_path=atlas_path,
        transforms=train_composed,
        img_size=img_size,
    )
    val_set = IXIBrainDataset(
        data_dir=val_dir,
        atlas_path=atlas_path,
        transforms=None,
        img_size=img_size,
    ...
    for epoch in range(epoch_start, epochs):
        ...
        for data in train_loader:
            ...
            # IXI dataset returns (x, y, x_seg, y_seg) — use first 2
            x = data[0].to(device)
            y = data[1].to(device)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train on IXI Dataset (Atlas-to-Patient)')
    parser.add_argument('--data_dir', type=str, required=True,
                        help='Root IXI data directory containing Train/, Val/, Test/')
    parser.add_argument('--atlas_path', type=str, required=True,
                        help='Path to atlas file (e.g., data/IXI/atlas.pkl)')
    ...
    parser.add_argument('--img_size', type=str, default="160,192,224",
                        help='Image size as H,W,D (default: 160,192,224)')
